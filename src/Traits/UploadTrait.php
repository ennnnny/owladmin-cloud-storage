...
trait UploadTrait
{
     ...
     * @throws \Exception
     */
    public function cloudSimpleUpload($file_path, $storage_id, $fileName = '')
    {
        //file exist？
        if (! file_exists($file_path)) {
            return null;
        }
        $storage = CloudStorage::query()->find($storage_id);
        if ($storage) {
     ...
     * @throws \Exception
     */
    public function cloudChunkUpload($file_path, $storage_id, $fileName = '', $chunk_size = 50 * 1024 * 1024, $min_size = 5 * 1024 * 1024 * 1024)
    {
        //file exist？
        if (! file_exists($file_path)) {
            return null;
        ...
        if ($storage) {
            $size = filesize($file_path);
            if ($size < $min_size) {
                return $this->cloudSimpleUpload($file_path, $storage_id, $fileName);
            } else {
                $service = new CloudUploadService;
                $config = $storage->config;
                if (empty($fileName)) {
                ...
                if (count($partList) > 0) {
                    ...
                    $service->store(array_merge($res, ['size' => $size]));
