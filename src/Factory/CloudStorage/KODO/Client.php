...
namespace Slowlyo\CloudStorage\Factory\CloudStorage\KODO;
...
class Client
{
    ...
    public function receiver(string $object,string $filePath): array
    {
        try {
            ...
            list($ret, $err) = $uploadMgr->putFile($token, $object, $filePath, null, 'application/octet-stream', true, null, 'v2');
            // 请求成功
    ...
    public function startChunk(string $object): ?string
    {
        try{
            $token = $this->kodoClient->uploadToken($this->config['bucket']);
            $uploadMgr = new UploadManager();
            list($ret,$err) = $uploadMgr->putFile($token, $object, null,
    ...
    public function chunk(string $uploadFile,string $object,string $uploadId,int $partNumber = 1,int $partSize = 0): array
    {
        try{
            ...
            list($ret,$err) = $uploadMgr->putFile($token, $object, $uploadFile,
                null,'application/octet-stream',false,
                null,'v2',$partSize);
            dd($ret,$err);
...
//            ));
            // 请求成功
            return array('eTag'=>trim($result["ETag"],'"'));
        }catch (\Exception $e) {
            throw new \Exception($e->getMessage());
        }
    }
    ...
    public function finishChunk(string $object,string $uploadId,array $partList): array
    {
        try{
            ...
            $result = $this->cosClient->completeMultipartUpload(array(
    ...
    {
        try{
            $result = $this->cosClient->listParts(array(
                'Bucket' => $this->config['bucket'], //格式：BucketName-APPID
                'Key' => $object,
                'UploadId' => $uploadId,
            ));
